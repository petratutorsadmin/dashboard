...
// Clean up mock lessons
let deletedMocks = 0;
for (const s of Object.values(db.students)) {
    if (s.lessons) {
        const initialCount = s.lessons.length;
        s.lessons = s.lessons.filter(l => !l.topic.includes('Practice Session') || l.topic.includes('IELTS Prep Practice Session') === false);
        deletedMocks += (initialCount - s.lessons.length);
    }
}
console.log(`Deleted ${deletedMocks} mock lessons.`);
...
for (const row of records) {
    ...
    if (!lessonExists) {
        ...
        updatedCount++;
        console.log(`Added REAL lesson for ${studentName} on ${formattedDate}`);
    }
}
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    console.log(`Successfully updated data.js. Added ${updatedCount} lessons.`);
} else {
